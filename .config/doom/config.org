...
** Which key
Reduce the popup delay of ~which-key~
#+begin_src emacs-lisp :tangle yes
...
** Window splitting
These changes make window splitting a bit nicer

First I want to focus to change to newly created windows.
#+begin_src emacs-lisp :tangle yes
