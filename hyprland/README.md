# My Arch Hyprland Setup

This is my arch hyprland setup

> [!WARNING]
> This setup is dependent on my dotfiles, so you can't run this folder separately

## Installation instruction

This setup works on the minimal installation of archlinux

> [!NOTE]
> While installing `archlinux` using `archinstall` make sure to select "Copy Network Settings from ISO" `or` "Network Manager (with iwd backend)", since I'm custom configuring my network settings.
